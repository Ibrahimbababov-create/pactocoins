...
// Вся система званий (13 рангов по total_earned) отключена по просьбе
// 2026-09-09 — «бесполезная». Код и таблица LEVELS оставлены на будущее.
// При выключенной системе last_level_id всё равно молча обновляется,
// чтобы при повторном включении не было задвоенного шквала поздравлений.
const LEVELS_ENABLED = false;
...
export async function checkAndApplyLevelUp(userId, admin) {
  const { data: profile } = await admin
    .from("users")
    .select("name, balance, total_earned, last_level_id, is_guest")
    .eq("id", userId)
    .single();
  ...
  if (!LEVELS_ENABLED) {
    // молча запоминаем достигнутый уровень, без наград/поздравлений/событий
    await admin
      .from("users")
      .update({ last_level_id: newLevel.id })
      .eq("id", userId);
    return;
  }
